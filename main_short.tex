...
respectively (a gravity/no-contact accounting audit places the control floor
below $10^{-3}$~J, so the $6.7$~J is real injection). A same-path iteration
ladder, equal-row-count schedules, warm starting, and mode truncation localize
this amplification to finite-budget convergence of the shared modal coupling,
and an in-host block-condensation ablation shows that condensing the shared
rows does not fix it. When the contact architecture is flexible, the tested
stiffness-aware implicit realization is the preferred observed control; in a
fixed XPBD host the local budget is best spent on iterations, with a verified
modal basis, since more substeps and a block solve do not suffice. For hosts
that still cannot converge the row, a cumulative modal-storage projection
enforces its scalar accounting bound unconditionally
(Prop.~\ref{prop:guarantee}, roundoff floor in all $90$ measured cells) but
opens up to $21.6$~mm of penetration ($72\%$ of the board's thickness); it is
therefore containment rather than corrected contact.
\end{abstract}
...
Governed, \eqref{eq:invariant} holds peak modal energy to $29.5$~J and the
books stay within $1$~mm of rest; the reference is the host's own
high-iteration run ($500{\times}1$). The bound removes the spurious launch and
some legitimate motion with it: the reference lifts one book $19$~mm, the
governed run none.}
\label{fig:teaser}
\end{figure}
...
is purely kinetic (translational and rotational, over all dynamic bodies, not
merely those in contact) and $\pm$ denote the two ends of substep $k$. We call
this the \textbf{measured gross rigid-side loss}: the scene-wide,
gravity-corrected, gross kinetic-energy loss of the rigid subsystem. It is a
loss, not a dissipation: it also counts legitimate rigid-to-modal transfer and
losses at rigid--rigid contacts elsewhere in the scene, so it is an envelope on
what contact dissipated rather than a measurement of it (\S\ref{sec:limits}).
Gravity enters only through $W_g$, so free ballistic motion registers zero
supply: a falling body's kinetic gain exactly equals the gravity work, and only
an inelastic velocity change funds the reservoir.
...
whose supply is measured rather than prescribed: the right-hand side is the
...
neither port passivity nor a signed per-interface bound. The gross sum ignores
substeps in which the rigid subsystem gains energy, and it counts energy
returned and re-dissipated as fresh supply (\S\ref{sec:limits} measures both).
We read two diagnostics against the sweep below and keep them distinct: the
incident-energy ratio $R=$~peak modal energy / peak incident rigid KE, a
...
bound requires a projection, which is deferred to \S\ref{sec:bound}.
...
puts that control floor below $10^{-3}$~J on both non-injecting hosts, three to
four orders under $6.7$~J, so the augmented-Lagrangian overdraft is real
impact-driven injection rather than accounting noise. These are three regimes
that the ratio-to-incident-energy diagnostic compresses into one number.
...
is the iteration budget, not how the shared support rows are scheduled:
replacing the serial per-row support projection with a joint block condensation
of the active rows (modal weight, implicit-midpoint restoring step, budget and
warm start held fixed; Fig.~\ref{fig:kconv}(c)) does not remove the injection.
The condensed variant is comparable at the starved $4{\times}1$ corner but
markedly worse at every higher budget where the serial path holds ($R=12.5$ and
$66.5$ at $32{\times}1$ on shelf and ledge, against serial's $0.30$ and
$0.09$). Reproduced over two scenes and four budgets, the ablation is a causal
probe that exposes a different and worse convergence path (a diagonal
rigid-body block approximation); it is not a cure.

\paragraph*{Robustness of the failure.} The catastrophic cells are not a
knife-edge: they survive a predefined local neighborhood. For each worst cell,
the equal-row pair, and the guardrail cell we run $16$ paired deterministic
perturbations, $12$ physical (drop height $\pm1$--$2\%$, normal speed, contact
offset $\pm1.5$~mm, tilt) and $4$ contact-row-order permutations, kept as
separate cohorts. Every injecting cell overdraws in all $16$, the sign never
flips, and the magnitude spread stays under one order of magnitude
($\log_{10}$ range $0.31$--$0.70$); the $4.4\times10^{7}$~J worst case is the
maximum of a $2.2$--$4.5\times10^{7}$~J band. The equal-row inversion
($32{\times}1$ holds, $4{\times}8$ injects, in all $16$) and the scene
dependence (never injecting on the table) survive as well. Mode truncation
localizes the amplification but does not remove it: excluding the stiff cluster
cuts the shelf $4{\times}1$ ratio $6333\to22.1$ ($584$~J still overdrawn), and
$6$ of $8$ injecting cells still overdraw (worst ledge $+1.24\times10^{6}$~J),
so band-limiting the co-solved basis is not sufficient.
...
and credit $\eta\max(\dErig,0)$ to $B$ in the same substep; if
$\Emod^{+}>\Emod^{-}+B$, scale the state by $\gamma$; debit $B$. Contact is not
re-solved. The form \eqref{eq:gamma} is closed because $\Emod$ is quadratic:
scaling $\mathbf{q}$ and $\dot{\mathbf{q}}$ together scales both terms by
$\gamma^2$, so it bounds total energy, not velocity alone, and it is inert
...
$c_k$ before the test caps realized storage at $\Emod^{-}+B_{k-1}+c_k$
...
\eqref{eq:invariant} itself, not a relaxation: the same-substep credit absorbs
the granularity term that a naive telescoping would leave.
...
$1.1\times10^{-13}$~J). The worst realized ratio falls to $1.22$
(position-based) and $0.87$ (augmented-Lagrangian). The projection is inert
wherever the solve is already within budget: every implicit cell, and
$16{\times}4$ and above on the position-based host at relaxation $0.7$.
...
next substep's normal impulse against the unclamped steady-state median,
reported as a ratio because the hosts' multipliers are different objects. The $4{\times}1$ cells
...
$196{\times}$ to $3.7{\times}$, and from $189{\times}$ to $3.6{\times}$ against
the XPBD self-reference, so the improvement does not depend on the choice of
reference. The trajectory, however, degrades: $\|d-d^{\text{ref}}\|_\infty$
rises from $6.5$~mm ($33\%$ of the reference peak) to $14.2$~mm ($71\%$). The
governor is a safety envelope, not an accuracy device. At the deployed
$1{\times}8$ cell the energy error falls from $2823{\times}$ to $3.7{\times}$
and the trajectory improves ($108\to96\%$ of the reference peak), though $96\%$
is still not faithful.

This explains how a $196\times$ energy error coexists with a peak deflection
only $1.2\times$ too large: $99.6\%$ of the excess sits in the shelf's stiff
mode cluster above $20$~kHz (${\sim}200\times$ the substep rate), which a
single scalar cannot separate from the legitimate low-frequency sag. The
projection removes the amount but not the character of the excess
($99.6\to97.7\%$ above the cut), scaling the sag ($24.1\to5.9$~mm against
$20.0$) down with the noise. The first-line remedy is band-limiting the basis,
not a scalar.

\paragraph*{Contact validity.}
Since $\gamma<1$ shrinks the sag $\mathbf{U}_y^{\!\top}\mathbf{q}$,
\eqref{eq:gamma} lifts the support into the resting body without
...
That worst case is $72\%$ of the $30$~mm board thickness and
$1.05$--$1.08\times$ the board's peak dynamic deflection: a visible
interpenetration exactly where the projection is most load-bearing. Contact
recovers on the next substep, but at a normal impulse up to $8.9\times$ the
steady-state value and a multiplier variance up to $62\times$ higher; rigid
momentum is unchanged (exactly zero, verified), since \eqref{eq:gamma} writes
only modal state. On the augmented-Lagrangian host the projection is an order
of magnitude gentler: at its one materially overdrawing cell ($+6.7$~J) it
opens $1.8$~mm against $21.6$, with a single clamped substep and no measurable
corrective impulse (full rows, and a decision table over these options, are in
the supplement).
...
unreduced FEM solve on the same discrete operator (so that discretization bias
...
its run-to-run variance, so we report the percentage). The relative cost is
larger where the budget is tight: at the deployed $1{\times}8$/$2{\times}4$
schedules the same ${\sim}0.3$--$2.2$~ms is $6.6$--$34.3\%$. We make no
unqualified real-time claim; an enforced device-resident $\gamma$ does not
exist in this implementation (\S\ref{sec:limits}).
...
opens contact violations up to $21.6$~mm and moves the trajectory away from
the implicit reference even as it moves the energy toward it
(\S\ref{sec:validity}); the result is bounded, not faithful. Its proper use is
as a safety envelope for budgets that would otherwise diverge; convergence is
the better remedy wherever it is affordable. The radial scale is not the only
admissible projection, however: the guarantee constrains where the projected
state lands, not the direction it moves. In preliminary tests a successor that
holds the contact-observed surface fixed, scaling only the directions the active
rows cannot see, enforces the same bound with $4.4$--$12.8\times$ less
worst-case penetration on three of four starved cells, though not on the fourth,
where the observed sag alone carries two orders more energy than the budget. A
...
${\lesssim}1\%$ expected and are partition-dependent, so the budget belongs to
the schedule as well as the scene. Enforcement is host-side and single-machine
(CPU float64); a device-resident governor and a governed-path FEM validation
remain future work. We test three formulations with one implementation of each,
and three scenes in one contact regime; the augmented-Lagrangian host still
overdraws in $3/24$ cells (up to $6.7$~J), so any claim that descent-class
solvers are passive here is empirical about this sweep, not a theorem.

% =====================================================================
\section{Conclusion}
% =====================================================================
For a developer who has already chosen a fixed-budget XPBD rigid-body host and
wants low-cost vibration on a stiff prop, the decisions are ordered; the
question is not XPBD versus another solver. First pick the representation:
full-space XPBD for large, local, or nonlinear deformation, and a compact modal
...
control we observed (it never overdraws), an implementation-specific
observation rather than a solver-class theorem. If the XPBD host is fixed, the
direct two-way modal row overdraws its measured supply by up to
$4.4\times10^{7}$~J at a few local iterations, and neither more substeps at
equal row cost, nor warm starting, nor condensing the shared support rows into
one block removes the overdraw (\S\ref{sec:kconv}); converging the row with
iterations does, so the budget is best spent on iterations, with a verified
basis. Only when a fixed host still cannot converge is the cumulative
modal-storage bound appropriate: it holds in all $90$ measured cells but opens
up to $21.6$~mm of penetration, so it is containment rather than corrected
contact. A low-dimensional modal extension is cheap in state size; it is not
automatically easy for a truncated local contact solve.
