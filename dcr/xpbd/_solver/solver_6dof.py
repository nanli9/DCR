...
class Solver6DOF:
    def __init__(
        ...
        # Per-body global-floor mask (1 ⇒ skip XPBD's solve_floor_contacts for
        # that body — used by reduced-coupled couplers that own their own
        # per-corner deformable-support anchors). Allocated in _flush().
        self.floor_disabled = None
    ...
    def _add_body(self, position, half_extents, mass, quaternion, velocity,
        ...
        self._floor_disabled_host.append(0)
        self._dirty = True
    ...
    def _flush(self):
        ...
        self.floor_disabled = wp.array(
            np.asarray(self._floor_disabled_host, np.int32), dtype=int, device=dev)
            ...
            for _it in range(self.iterations):
                ...
                wp.launch(K6.solve_floor_contacts, dim=n,
                          inputs=[self.x, self.q, self.x_prev, self.q_prev,
                                  ...
                                  self.floor_disabled, self.floor_y, mu,
                                  self.lam_floor, h],
                          outputs=[self.dx, self.drot, self.dcount], device=dev)
