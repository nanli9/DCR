"""G1 — build a native modal support from TRUE FEM eigenmodes.

Ported from the `benchmark` branch's X3 harness
(`benchmarks/paper_eval/x3_ground_truth/fem_modal_support.py`) into the
library: this is the shared-operator arm (validation plan G1 /
docs/benchmark_plan.md §4). The production scenes normally install a
SYNTHETIC analytic basis (`make_debug_reduced_shelf_support` — sine bending
modes + Gaussian contact bumps) whose spatial falloff is partly
bump-PLACEMENT-shaped and whose effective stiffness is NOT the scene's
plate. This module rebuilds the same `ReducedSupport` container from the
true FEM eigenmodes of the slab (`dcr/modal/modal_analysis.ModalAnalysis`,
paper Eq. 6 `Kψ = ω²Mψ`), so the native arm is literally a MODAL REDUCTION
of the very FEM operator the all-FEM ground truth (`MultiFEMSim`)
integrates. Compared head-to-head, the only differences are (a) modal
truncation to k modes and (b) the host contact model.
