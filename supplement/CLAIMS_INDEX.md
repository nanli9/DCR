...
| §3.1, Fig. 2 bottom | Eq. (2) violated in 23/24 AVBD cells, 8/24 XPBD, 0/24 impulse; AVBD worst 15.1 J | `data/eq2_utilization.csv` | `run_eq2_utilization.py --check-frozen` |
| §3.1 | governed: all 72 cells satisfy Eq. (2); worst ratio 1.22 / 0.87 | `data/solver_matrix.csv` | `run_solver_matrix.py` |
| §3.2, Fig. 3 | XPBD 2.96e4 (K=1) -> 0.300 (K=32); reference 0.2735; implicit spread 4.9e-4 over K=2...500 | `data/k_convergence.csv` | `run_k_convergence.py` |
...
| §3.2 | deployed 1x8 / 2x4: XPBD violates 6/6 (worst R = 2282), AVBD 5/6 by <= 0.16 J, impulse 0/6 | `data/eq2_deployed.csv` | `run_eq2_utilization.py --budgets 1x8,2x4 --relaxes 0.7 --out eq2_deployed` |
| §3.2 | governed at the deployed budgets: all 18 satisfy Eq. (2), worst R = 1.022 | `data/solver_matrix_deployed.csv` | `run_solver_matrix.py --budgets 1x8,2x4 --relaxes 0.7 --out solver_matrix_deployed` |
...
| §3.3 | AVBD is gentler in absolute terms (worst 1.4 and 3.1 mm) and comparable in relative terms (3.1x and 5.5x) | `data/projection_validity_avbd.csv` | `run_projection_validity_avbd.py` |
| §3.3 | accuracy at shelf 8x2: 1555.6 J -> 29.26 J against 7.92 J; energy error 196x -> 3.7x; trajectory 6.5 mm -> 14.3 mm | `data/governed_accuracy.csv` | `run_governed_accuracy.py` |
...
| §3.5 | device-resident monitor path: 5.0-8.9 ms/step at 16x4 | `data/perf_device.csv` | `(x5_perf device harness; see ledger)` |
