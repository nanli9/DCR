...
**The second half of the slide/spin fix: tangential contact friction (added 2026-06-11).** The anchor low-pass above kills the *impact* angular kick, but it leaves a separate gap that the velocity band then exposed. The coupler's contact constraint is **normal-only** (`J_x = [ŷ; r×ŷ]`, gap `g = corner_y − anchor_y`), and tracked bodies are taken off the host solver's global floor (`floor_disabled`) because the coupler owns their per-corner contact. So a resting body has **no tangential channel at all**: lateral momentum is never dissipated (constant slide) and angular momentum about the support normal (yaw) is never dissipated (constant spin). The velocity band makes the spin dramatic — each band impulse is `λ n̂` applied at an offset corner, so it carries an angular impulse `Δω = I⁻¹(r×n̂)λ`, and with no frictional sink these accumulate into a steady ~3.8 rad/s yaw (measured: band-off yaw `6×10⁻³` *growing* → band-on yaw locks at `3.8` and never decays). The §15 energy bound does **not** forbid this — a steady bounded yaw has bounded KE, so passivity holds while the body spins forever. The fix is the standard, additive one: a **velocity-level dynamic Coulomb friction pass** at the in-contact corners (Müller 2020 Eq. 30), written in the band's own impulse formulation — per corner remove the tangential corner velocity, clamped to the cone `μ·λ_n` with `λ_n = (m g / n_c)·h` the gravity-support impulse share. It reuses the solver's existing `μ` (**zero new knobs**), and because `U_y` is normal-only the friction impulse is **orthogonal to the ring** — it does not enter the modal energy ledger, so the §15 bound and the band's passivity are untouched. Measured (shelf, η-sweep 0→1): lateral `v` and yaw `ω_y` settle from `~0.2 / ~3.8` to `~5×10⁻⁶`, while `qd_peak` is unchanged (ring still rings) and the per-prefix reservoir stays ≥ 0 at every η. Authored numpy-first (`apply_contact_friction`), then ported to a full Warp kernel (`k_contact_friction`, fully GPU-resident, launched after the band inside the captured substep graph); the kernel is parity-checked bit-for-bit against the numpy reference on the Warp CPU device. *(`# DEVIATION` foundation §15 / paper Eq. 10: friction is a body-side contact term orthogonal to the modal coupling; it is **not** the modal tangential coupling of §7 — `−Φ(x_s)q̇` still has no tangential column.)*
...
**Status / plan.** **V0 (done, validated):** the passivity bet, on the standalone prototype + viser (`uv run python scripts/run_impulse_port_v0_viser.py`, toggle coupling velocity↔position). **V1 (done — wired + tested):** `reservoir_alpha`/`reservoir_draw` (the reservoir-exact governor, §3.4) wired into both the V0 prototype (`governor_mode="reservoir"`) and the scene band (`apply_velocity_band`, reservoir persisted on the coupler); `tests/impulse_port/test_v1.py` (26/26 incl. V0) asserts the per-prefix bound holds for η∈{0.1,0.3,0.5,1.0}, never clamps at η=1, and that the old per-impulse cap dips negative where the reservoir stays ≥0 (`docs/impulse_port_v1_reservoir.png`). **V2 (generalized via host pass; device fold pending):** `apply_velocity_band` is solver-agnostic and runs on XPBD *and* AVBD through one code path (§3.3, verified 446/404 impulses) — but as the numpy host pass, not yet folded into the GPU substep loop; `anchor_includes_q_d`/`F_q_dyn` not yet retired. **V3:** the §4 validation re-run on the now-faithful config.
...
- Friction coupling: **body-side tangential Coulomb friction now exists** (`apply_contact_friction` + the GPU-resident `k_contact_friction`, §3.1) — it dissipates the lateral slide and yaw spin a resting body would otherwise keep forever, reusing the solver's `μ` with no new knob. What it does **not** yet do is couple friction to the *modal* DOF: `J_q = −Φ(x_s)` still carries only the normal column, so tangential friction doesn't see `−Φ(x_s)q̇` (a sliding body doesn't feel the surface's tangential ring motion, and vice-versa). That full 3D moving-support modal-friction coupling is the remaining kernel-level change. First write-up, or follow-up?
