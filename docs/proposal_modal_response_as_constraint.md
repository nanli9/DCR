...
| **XPBD** (compliant constraints) | yes, very naturally — `q` is a compliant DOF (compliance `1/k_modal`); the constraint gradient already carries the `J_q` column | yes, in the iterate→∞ limit | arguably the *cleanest* host; transpose-consistency is automatic from the single constraint gradient. **Now implemented** (`dcr/xpbd/reduced_coupled_xpbd.py`) — the port is a thin contact-force adapter, see §6.1 |
...
**Honest framing for the write-up.** "Reduced modal response as a constrained co-DOF" is a general recipe for constraint-based dynamics, with two caveats: (i) *exactness* (true equilibrium, not penalty-biased) needs monolithic coupling — Schur or compliant-projection, not plain block-Gauss-Seidel; (ii) AVBD enforces contact by *augmented Lagrangian* (`f = λ + k·g`, multiplier updated each iteration), so at convergence the constraint is met at **finite** `k` with no steady-state penalty bias — but the per-iteration Hessian still carries the penalty stiffness `k`, and when `k` greatly exceeds the modal stiffness the coupled block is ill-conditioned, so at a *finite iteration budget* the recovered `q` deviates from the analytic `K_q⁻¹F` and `q ∝ 1/E` scaling degrades. This is a *conditioning / finite-iteration convergence* property of stiff penalised/AL contact (not a steady-state penalty bias), pinning the trustworthy regime to stiff supports (`E ≳ 10¹⁰ Pa` for thin shelves; consistent with the paper's own Table 2 using ~10 GPa for most models). A hard-constraint solver avoids the conditioning issue but trades it for stiffness.

> **Sustained rotational limit cycle — root cause (isolation-tested).** A body resting where an *excited* support bending mode has nonzero **slope** across its contact footprint develops a **sustained rotational limit cycle** (~0.1–1°) of the contact–modal coupling. Isolation sweeps (boulder-only on the support, modal basis held fixed, CPU-deterministic) pin **three necessary ingredients**: (i) **off-peak / sloped contact** — at a mode peak the two contact corners sit at equal height and the rock is *identically zero* (a boulder at the beam centre, and the centred dinner-table pot, both measure 0.000° even under a hard off-axis impact); off-peak it rocks, peaking partway to the edge. (ii) **enough impact to excite the mode** — a threshold ~2 m/s, then *non-monotonic / chaotic* in magnitude (a basin, not a resonance), reachable by **either** initial velocity **or** drop height (e.g. `drop=1.26 m` rocks while `2.0/4.0 m` happen to miss) — so there is **no "safe" energy knob**. (iii) **no dissipation on that channel** — the body couples only to the *undamped algebraic* `q_s`, not the damped `q_d`, so once kicked into the rocking torque there is no bleed-off and it sustains, slow-growing over ~8 s. It is 100% coupling-sourced (vanishes on a rigid floor) and numerical (support deflection is sub-mm, so a ~1° tilt is not physical), and no loop-gain lever removes it (anchor low-pass τ, iteration count, cross-block under-relaxation). **Load and stiffness are basin-wideners, not the cause:** the boulder *alone* at 50 kg already rocks; heavy mass (120 kg), a soft support (shelf `E=5e8`), and neighbouring bodies sharing the global `q_s` (~2×) each lower the threshold / raise the amplitude, but none create the attractor — a stiffer support (steel `E=2e11`) or lighter load merely narrows the basin. (So an earlier "heavy-mass load/stiffness boundary" reading was incomplete: the root is the **undamped `q_s` torque under off-peak sloped contact**, with load/stiffness only setting basin width.) The only robust cure attacks ingredient (iii) — a dissipation path on the `q_s` coupling channel — which is exactly the **energy-bounded enforcement** direction (milestone 3); the basin-width vs load/stiffness map is a clean cost/regime characterization target (milestone 6).
...
### 6.1 What actually differs across solvers: a solver-agnostic core + a thin contact-force adapter (AVBD and XPBD, both implemented)

The generality claim above is no longer hypothetical: the same constrained-co-DOF coupling now runs on an **XPBD backend** (`dcr/xpbd/reduced_coupled_xpbd.py`) beside the AVBD one (`dcr/avbd/reduced_coupled_avbd.py`). Diffing the two couplers line-for-line is the cleanest possible evidence for what is and isn't solver-specific — and the answer is sharper than "mostly identical because both are constraint-based." It is: **the coupling is a solver-agnostic core, and exactly one quantity is solver-specific.**

**The core is shared — structurally identical, and literally the same state + IIR machinery.** Both couplers import the same `ReducedSupport` (`q_s, q_d, q̇_d, K_q, M_q, D_q, Φ`) and reproduce, block-for-block, the same:
- gap and Jacobians — `g = corner_y − (y_rest + U_y·q_s)`, `J_x = [n̂; r×n̂]`, `J_q = −U_y` (the y-row of `Φ` at the contact point, i.e. `J_q = −Φ(x_s)` restricted to the normal);
- per-body 6×6 inertial Hessian `H_x = diag(m/h², I_world/h²)` plus contact Hessian;
- modal block `H_q = K_q + Σ_j k_j U_y U_yᵀ`, gradient `g_q = K_q q_s − Σ_j U_y f_j`;
- cross-block `M = ρ J_x U_yᵀ`, Schur reduction `S = H_q − Σ Mᵀ H_x⁻¹ M`, solve for `Δq_s`, back-substitute `Δx_i = −H_x,i⁻¹(g_x,i + M Δq)`;
- static/dynamic split + exact-resonator IIR step for `q_d`, and the contact-anchor static low-pass (§3.1).

None of that is XPBD- or AVBD-specific. It *is* the method of §2–§3.

**Exactly one thing differs: how the solver hands the coupler the per-contact normal force `f_j` and effective contact stiffness `k_j`.** Everything downstream — project `f_j` onto `U_y`, build `H_q`/`g_q`, Schur-reduce, drive the ring — is identical code-flow. The two backends answer that one question in completely different ways:

| | **AVBD** adapter | **XPBD** adapter |
|---|---|---|
| stiffness `k_j` | the solver's **escalating** augmented-Lagrangian penalty `ρ = c_penalty`, clipped at `rho_clip = 1e9` (`reduced_coupled_avbd.py:1083,1172`) | a **fixed** `contact_stiffness = 1e8` (`reduced_coupled_xpbd.py:189`) |
| force `f_j` | read directly as `clip(ρ·C + λ, f_min, f_max)` — the AL **dual** `λ = c_lambda` is owned and ascended by the AVBD kernel; the coupler only *reads* it (`:1082–1178`) | **recovered post-solve** from the body momentum deficit `f_body = −(m/h²)(x_final − x_predicted)`, split across the body's rows (`:696–716`); there is **no dual** (`_lambda_per_row` is inert) |
| conditioning | needs `rho_clip` + Schur regulariser auto-scaled `ε ∝ ρ²/m`, because `ρ` blows up across iterations | none — a fixed penalty never escalates, so no clip and no auto-`ε` |

That table is the entire "solver port." The honest generalization is to *name it*: a **`ContactForceProvider` adapter** that, per tracked contact row each iteration, returns `(C, f_j, k_j, J_x)` and accepts the writeback `(Δx, Δq_s)`. Two such adapters now exist; a third solver plugs in by implementing only that. (AVBD's adapter is the larger of the two, but only because escalating `ρ` *forces* the extra clip/auto-`ε` machinery — the conceptual surface is the same few quantities.)

**Why it's "mostly," not "fully," identical — and why that difference is exactly the exactness column.** The distinction is not cosmetic; it is precisely what the §6 table's "reaches true `K_q⁻¹F`?" column tracks:
- AVBD exposes a **dual-stabilised** force `ρC + λ`. The dual `λ` cancels the penalty's compliance error, so the force is accurate *at finite `ρ`* — which is why AVBD hits `K_q⁻¹F` and is the reference path.
- XPBD exposes a **fixed-compliance** force recovered from kinematics. Its accuracy is the compliant fixed-point accuracy: exact only in the iterate→∞ (equivalently stiffness→∞) limit. That is what the table's "yes, in the iterate→∞ limit" entry *means*, now shown to be a structural property of **where the force comes from**, not an implementation accident.

**The genuinely harder case — velocity-level / sequential-impulse solvers.** Both backends here are *position-level* solvers that (a) expose a **per-iteration position callback** (`substep_begin / iteration / substep_end`) and (b) tolerate an external block editing their DOFs mid-solve. That callback contract is itself part of the interface. A sequential-impulse / velocity-LCP solver (Bullet-style PGS) offers neither: there is no position iteration to hook, and the contact "force" lives as an accumulated normal impulse `λ_n/h²`. Hosting the *monolithic* coupling there is **not** a thin adapter — you would recast the coupling at the **impulse/velocity level** (inject `Δq̇` through `J_qᵀ` directly, rather than solving a position `Δq_s`), a related but distinct formulation. So the honest boundary is: *inside* "position-level primal/compliant solver with a per-iteration position hook," the port is the few-quantity adapter above (AVBD↔XPBD proves it); *outside* it, the generalization is a reformulation, not a drop-in.

**Status — demonstrated vs. not.** The XPBD coupler is implemented as a numpy reference and smoke-tested (`tests/xpbd/`); the structural identity above is verified by reading both couplers, not asserted. What is **not yet** run on the XPBD path is the quantitative §4 battery (the `K_q⁻¹F` distance, the iteration/substep robustness sweeps) — that is exactly milestone 4's remaining deliverable, now de-risked because the coupling is *shared structure* rather than a re-derivation. (Separately, the XPBD *base solver's* tangential friction had a normal-impulse under-count that let resting objects slide; it is fixed in `dcr/xpbd/_solver/kernels_6dof.py` and is orthogonal to the coupling, which is normal-only — noted here only so an XPBD-vs-AVBD scene comparison starts from matched contact behaviour.)
...
4. **Solver-generality study.** The constrained-co-DOF coupling is now **implemented on a second solver family** — XPBD (`dcr/xpbd/reduced_coupled_xpbd.py`) — and diffing it against the AVBD coupler isolates the solver-specific surface to a single contact-force/stiffness adapter (§6.1): the coupling core (gap, Jacobians, `H_q`, cross-block, Schur, IIR) is shared; only "how the solver yields `f_j, k_j`" differs (AVBD reads the AL dual+penalty; XPBD uses fixed compliance + momentum-deficit force recovery). *Remaining:* run the §4 exactness/robustness battery on the XPBD path to confirm the exactness *tracks the coupling structure* (dual-stabilised → `K_q⁻¹F`; fixed-compliance → exact only in the iterate→∞ limit), and decide whether the velocity-level/LCP case is needed for print.
...
- The **XPBD re-implementation now exists** and reduces the cross-solver delta to a few-quantity contact-force adapter (§6.1). Is that — *position-level* AVBD + XPBD, one dual-stabilised and one fixed-compliance — enough to support the "universal for constraint-based solvers" claim, or do you want the **velocity-level/LCP** case (where it becomes an impulse-level reformulation, not a drop-in adapter) before we put it in print?
...
- The **exact coupling** transfers to any solver that couples the modal block monolithically (Schur / XPBD-compliant); a block-Gauss-Seidel solver hosts it but only approximately (that's the ~70%-off baseline). **Now demonstrated on a second backend:** the coupling runs on XPBD too, and diffing the two couplers shows the *entire* solver-specific surface is a thin contact-force adapter — the gap, Jacobians, `H_q`, cross-block, Schur and IIR are shared code; only "how the solver yields the contact force/stiffness" differs (AVBD reads its AL dual+penalty; XPBD uses fixed compliance + momentum-deficit recovery). §6.1.
...
- Working prototype on AVBD, now ported to XPBD (same coupling core, thin per-solver contact-force adapter — §6.1). Want your steer on the headline (exactness vs. energy-conservation) and how far to push the generality claim before publishing (position-level AVBD+XPBD already in hand; velocity-level/LCP would be a reformulation, not a drop-in).
```
```
