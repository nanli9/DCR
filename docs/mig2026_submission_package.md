...
| `data/` | the four result CSVs + JSON manifests from `benchmarks/paper_eval/x1_passivity/out/` | ready (committed) |
| `README.txt` | blind-safe version of §1 + §2 | draft in §4 |

Keep the whole archive under 200 MB; the CSVs are a few hundred kB, so the
video is effectively the entire budget. Target ≤ 150 MB encoded.

---

## 4. Blind-safe repro note (paste-ready for the submission)

> All reported measurements were produced on a single consumer laptop (10-core
> ARM64, 16 GB, CPU only) with CPython 3.12 and numpy 2.4. Results from a second
> x86 machine are deliberately never combined with these: chaotic contact stacks
> diverge between architectures under floating-point reassociation. The native
